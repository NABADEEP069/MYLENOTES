...
const UploadForm: React.FC<UploadFormProps> = ({ onClose }) => {
  ...
  const inputStyles = "w-full h-10 border text-gray-800 text-sm border-text rounded-lg px-2";
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-background bg-opacity-50 z-50">
      <div className="relative w-2/6 h-auto p-6 flex flex-col items-center justify-center bg-background border border-text rounded-lg">
        <form className="w-full flex flex-col items-start">
          ...
          <textarea id="description" className="w-full h-36 border text-sm text-gray-800 border-text rounded-lg px-2" />
        </form>
        ...
        <div className="flex items-center justify-evenly w-full mt-4">
          ...
          <button className="bg-primary text-text p-2 px-4 rounded-md">Submit</button>
        </div>
